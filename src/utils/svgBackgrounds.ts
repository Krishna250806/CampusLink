...
export const resolveSvgPattern = (patternIdOrDataUrl?: string): string => {
  if (!patternIdOrDataUrl) return '';
  if (patternIdOrDataUrl.startsWith('data:') || patternIdOrDataUrl.startsWith('http') || patternIdOrDataUrl.startsWith('/')) {
    return patternIdOrDataUrl;
  }
  const preset = SVG_BACKGROUND_PRESETS.find(p => p.id === patternIdOrDataUrl);
